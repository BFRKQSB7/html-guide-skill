...
5. **清理临时文件（必做，硬规则）**：本流程产生的**所有**过程产物——无头验证的 `dom*.txt`、
   长图/摘要卡的 capture 样式临时副本（`<name>-capture.html` 等）、中间截图、make_a4 的
   `<name>-a4.html` 分页源、脚本中间文件——用完后**一律删除**，不允许「放临时目录凑合」。
   输出目录只留最终交付物（`.html` + 用户勾选的长图/摘要卡 png；`<name>-card.html` 若打算
   保留作再编辑源可留）。**交付前 `ls` 输出目录复核**，多出的文件当场删，不留到下次。
   评测反馈点名过临时文件混进成品。
...
5. **清理**：截图完成后删除注入 capture 样式的临时副本（只留长图 png）——见 Step 5.5 硬规则
...
5. **清理**：裁剪完成后删除 `<name>-a4.html` 分页源（只留 `page-NN.png`）——见 Step 5.5 硬规则
...
3. 无头截图成图（卡片版式固定 1200×630，但**导出用 2× 高清**——历史教训：1× 导出 1200×630 在手机/高分屏
   ...
   - 需要更清晰可提到 scale 3（3600×1890）；校验"内容一屏装下"始终以 1200×630 的 CSS 版式为准（DPR 只改
     像素密度不改布局）
4. **内容长度与自动缩字号**：模板内置防溢出脚本——正文超过 1200×630 可用高度时自动按 0.05 档
   整体缩小大标题/小标题/要点/注释字号（块面积随字号同步缩小），正常短内容不动；截图前无需手动算字。
   但别依赖它硬塞长文（图只装「一眼看完」的内容）：
   - **data 布局实测上限（中文、有注释时）**：大标题 h1 ≤4 行 ≈64 字；结论 ≤180 字；
     每条要点 ≤3 行 ≈105 字、全部要点总行数 ≤8
   - **经验法则**：标题+结论+全部要点总字数 ≤600 字号基本不动；≈800 自动缩到最小档仍装下；
     再往上会溢出，**必须删要点/压缩表述**，别指望缩字号救场
5. 陌生名词用底部 `.gloss` 注记一行；卡片必须一屏装下（校验以 1200×630 的 CSS 版式为准）
6. **清理**：截图完成后检查输出目录，卡片 png 之外的过程产物（中间截图、校验残留）一律删
   ——`<name>-card.html` 若想留作再编辑源可保留；见 Step 5.5 硬规则
