...
1. 隐藏交互构件（目录 / 按钮 / 进度条）后整页截图：`--screenshot=long.png --window-size=1280,<全高>`
2. 超高页面：分段截（重叠 ~200px）再纵向拼接
3. 长图给「整体印象」；要耐读/可打印用 6.2 的 A4 分页

#### 6.2 多图（A4 分页，一张一页，尺寸统一）

**一键脚本**（已内置本页全部规则：统一排版/表格不切/段落跨页/作者页脚/末页居中/标题防爆开）：
```bash
python scripts/make_a4.py <输入.html> <输出目录> [--style magazine] [--light|--dark]
```
