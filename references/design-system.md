...
- 需要**行号**时给 `<pre>` 加 `class="linenos"`；默认不加行号（与顶级文档站一致，
  行号只在需要逐行引用时开，见 4.27）
- **纯文本 / prompt 块用 `lang-text`**：长行会自动换行（`white-space:pre-wrap`），
  不会出横向滚动条——写提示词模板、长英文段落这类「按内容换行比保持单行更重要」的块用它；
  真正的代码（bash / python 等）保持默认不换行、横向滚动（缩进敏感，别硬换行）。
  历史教训：提示词模板里写 117+ 字符的长行，默认不换行 → 横向溢出 + 复制按钮挤压，观感「崩坏」。
...
- **非常见外语缩写也符合注记要求**：中文读者未必秒懂的西文缩写（TL;DR、EOL、RTFM…）首现时同样给 `.term` 注记（展开全称 + 中文含义），别默认读者认识；全球通用的常见缩写（URL、API、CPU、PDF）不必注。
