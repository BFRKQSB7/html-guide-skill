...
![](https://img.shields.io/badge/version-v2.4.0-blue)
...
### v2.4.0
- **优化**：摘要卡防溢出自动缩字号 + 内容长度上限文档

### v2.3.0
- **新增**：标签超市骨架同步分类「加载全部」（底部按钮完整加载该分类全部标签，分块渲染）
- **优化**：标签超市模板瘦身为骨架（13.9MB → 62KB）
- **优化**：SKILL 临时文件清理升级为硬规则（Step 5.5 全量覆盖，6.1/6.2/6.3 各补清理步骤）

### v2.2.x
- **新增**：headless 导出 PDF 选项 + 技能介绍页 GitHub Pages 托管
- **修复**：摘要卡 steps 字号 bug、prompt 代码块横向溢出
- **优化**：清理发布文件本机个性化信息（用户名路径 / 代理端口）

### v2.1.0
- **新增**：9 套设计师血统主题 + 摘要卡 6 布局（1200×630）+ A4 分页成图脚本

### v2.0.x
- **新增**：无障碍增强 + 阅读体验升级 + 主题过渡（v2.0.0）
- **新增**：得分柱状图 / 饼图 / 环形图组件（v2.0.1 / v2.0.2）
- **新增**：信息收集时效性方法论 + lint 强化 + 浏览器降级（v2.0.3）
- **优化**：代理端口解耦，个性化配置进 user-config.md（v2.0.4）
- **优化**：主题切换按钮悬浮说明、默认跟随系统（v2.0.5 ~ v2.0.9）

### v1.0.0
- **版本**：首个发布 —— 把内容生成自包含可视化 HTML 指导文档
